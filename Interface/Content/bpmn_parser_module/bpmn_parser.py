...
class BpmnParser:
    ...
    def process_bpmn(self, bpmn_path, extra=None):
        try:
            tree = ET.parse(bpmn_path)
            root = tree.getroot()
            diagbp_tag = root.find('.//' + TAG_NAME)
        except FileNotFoundError as e:
            print(f"-----ERROR-----: bpmn file not found: {bpmn_path}")
        ...
        bpmnGraph.load_diagram_from_xml_file(bpmn_path)
        ...
        with open(bpmn_path+".json", "w") as outfile:
        ...
        if os.path.exists(extraPath):
            ...
            diagbp_str = diagbp_str.replace('<'+TAG_NAME+'>', '').replace('</'+TAG_NAME+'>', '')
    ...
    def generate_json_from_bpmn_diag(self, bpmn_path):
        try:
            tree = ET.parse(bpmn_path)
            root = tree.getroot()
            diagbp_tag = root.find('.//' + TAG_NAME)
