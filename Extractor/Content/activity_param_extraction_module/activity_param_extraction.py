...
class ActivityParamExtraction:
    ...
    Il numero di timestamp è rilevato per-occorrenza: nella stessa traccia possono coesistere
    attività con solo start+complete e attività con assign+start+complete.
    """
    ...
    def preprocess_log(self) -> pd.DataFrame:
        ...
        try:
            ...
            # 5. Riordina eventi per creare timestamp strutturati (per-occorrenza)
    ...
    def _has_waiting_data(self) -> bool:
        """Controlla se il log processato contiene occorrenze con assign_timestamp."""
        return (
            self._processed_log is not None
            and 'assign_timestamp' in self._processed_log.columns
            and self._processed_log['assign_timestamp'].notna().any()
        )
    ...
    def extract_all_parameters(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. Estrazione distribuzioni tempo di attesa (solo se ci sono occorrenze con assign)
            waiting_file = None
            if self._has_waiting_data():
    ...
    def get_extraction_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_waiting_times': self._has_waiting_data()
        }
